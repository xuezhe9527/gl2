    ...
    async pay() {
          ...
          beforeClose:(action,instance,done)=>{
            ...
            if(action === 'confirm'){
              ...
              if(this.status ===200){
                ...
                this.$message.warning('请确认您已经支付成功')
              }
            //如果点击了取消按钮
            }else{
                  clearInterval(this.timer)
                  this.timer=null
                  this.$message.warning('支付中遇到了问题，请致电110')
                  done()
            }
          }
        })
          .then(() => {})  //如果直接在这里处理无论如何都会直接关闭对话框，所以需要在beforeClose中处理
          .catch(() => {});
      } catch (error) {
      ...
      if(!this.timer){
        this.timer = setInterval(async () => {
          ...
          if(result.code===200){
            this.status =200 //保存支付状态，在beforeClose中判断
            alert('支付成功,即将跳转至支付成功页面')
            clearInterval(this.timer)
            this.timer=null
            this.$router.push('/paysuccess')
            this.$msgbox.close()
